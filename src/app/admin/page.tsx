...
export default function AdminPage() {
  ...
  function handleEdit(p: Product) {
    ...
    setForm({
      ...
      category: p.category as FormData['category'],
      tag: p.tag || '',
      amazon: p.amazon,
      img: p.img,
    })
  ...
  if (!user) {
    return (
      <div className="admin-page" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', color: 'var(--fg)', fontFamily: 'var(--font-mono)' }}>
        <form onSubmit={handleLogin} className="admin-login-form" style={{ border: 'var(--border-width) solid var(--border)', maxWidth: '400px', width: '100%' }}>
  ...
  return (
    <div className="admin-page" style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--fg)', fontFamily: 'var(--font-mono)' }}>
      <header className="admin-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem 2rem', borderBottom: 'var(--border-width) solid var(--border)' }}>
        <div className="logo">GDL Admin</div>
        <div className="admin-header-right" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <span className="admin-user-email" style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>{user.email}</span>
          <button onClick={handleLogout} className="btn btn-sm">CERRAR SESIÓN</button>
        </div>
      </header>

      <div className="admin-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', padding: '2rem' }}>
        <div className="admin-form-panel" style={{ border: 'var(--border-width) solid var(--border)', padding: '2rem' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value as FormData['category'] })} style={{ width: '100%', background: 'transparent', border: 'var(--border-width) solid var(--border)', padding: '1.2rem', color: 'var(--fg)', fontFamily: 'var(--font-mono)', outline: 'none', marginBottom: '1rem' }}>
        ...
        <div className="admin-products-panel" style={{ border: 'var(--border-width) solid var(--border)', padding: '2rem', maxHeight: '80vh', overflowY: 'auto' }}>
          ...
          {products.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {products.map(p => (
                <div key={p.id} className="admin-product-item" style={{ display: 'flex', gap: '1rem', border: 'var(--border-width) solid var(--border)', padding: '1rem', alignItems: 'center' }}>
                  ...
                  <div className="admin-product-info" style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 'bold', textTransform: 'uppercase', fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                    <div style={{ color: 'var(--muted)', fontSize: '0.75rem' }}>{p.category} · ${p.price.toLocaleString()}</div>
                  </div>
                  <div className="admin-product-actions" style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
